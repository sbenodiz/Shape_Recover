...
cGuess=cReal;
dGuess=dReal;
RealGuess=[aGuess,bGuess,dGuess];
Guess=[0 0 30];
...
for i=FirstPointmirror(1):stepMirror:SecondPointmirror(1)
    for j=FirstPointmirror(2):stepMirror:SecondPointmirror(2)
        ...
        [m,n,parameters]=GetNextPixel_parameters(movi,pMiror1(2),pMiror1(1),WindowSize,IncludeIdentity,NormalizeMinimumFounded);
...
save('DataForOptimization');
 load('DataForOptimization');

%%%% run once


tic
 [bestGuess]=fminsearch(@(args) CheckParameters(args,AllData,K,Points3DScreen),RealGuess);

  [sumError,numOfPoints] =CheckParameters(RealGuess,AllData,K,Points3DScreen)
toc
...
DrawPlane(RealGuess,FirstPointmirror,SecondPointmirror,K,'r');
